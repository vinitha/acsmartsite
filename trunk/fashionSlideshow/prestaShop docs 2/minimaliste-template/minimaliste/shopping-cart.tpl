	...
	<form action="{$smarty.server.PHP_SELF|htmlentities}" method="post" id="voucher">
		<fieldset>
			...
			<p>
				...
				<input type="text" id="discount_name" name="discount_name" value="{if $discount_name}{$discount_name}{/if}"></input>
			</p>
			<p class="submit"><input type="submit" name="submitDiscount" value="{l s='Add'}" class="sub" /></p>
		</fieldset>
	</form>
</div>
{/if}
...
<p class="cart_navigation">
	<a href="{$base_dir}order.php?step=1" class="buttoncontinu" title="{l s='Next'}">{l s='Next'} &raquo;</a>
	<a href="{if $smarty.server.HTTP_REFERER && strstr($smarty.server.HTTP_REFERER, 'order.php')}{$base_dir}index.php{else}{$smarty.server.HTTP_REFERER}{/if}" class="buttonnext" title="{l s='Continue shopping'}">&laquo; {l s='Continue shopping'}</a>
</p>
{/if}

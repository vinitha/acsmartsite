...
ModalPanel.prototype = {
	initialize: function (id, options) {
		...
		var eDiv = $(this.div);
		if (eDiv.style.setExpression) if (ModalPanel.disableSelects || Richfaces.getComputedStyle(eDiv, "position") != "fixed") {
	...
	initIframe: function () {
		if (this.contentWindow) {
			Element.setStyle(this.contentWindow.document.body, {
	...
	processAllFocusElements: function (root, callback) {
		var idx = -1;
		var tagName;
		if (root.focus && root.nodeType == 1 && (tagName = root.tagName) && (idx = this.formElements.indexOf(tagName.toLowerCase())) != -1 && this.formElements.charAt(idx - 1) === '|' && this.formElements.charAt(idx + tagName.length) === '|' && !root.disabled && root.type != "hidden") {
	...
	show: function (event, opts) {
		...
		jQuery("<a class='closeBtn' href='#close'><span>close</span></a>")
			.click(function(e){
				e.preventDefault();
				overlay.destroy();
			})
			.appendTo(jQuery("#panelCDiv"));
		...
		if (!this.shown && this.invokeEvent("beforeshow", event, null, element)) {
			...
			if ((ModalPanel.disableSelects || this.options.overlapEmbedObjects) && !this.iframe) {
				...
				new Insertion.Top(eCdiv, "<iframe src=\"javascript:''\" frameborder=\"0\" scrolling=\"no\" id=\"" + this.iframe + "\" " + "class=\"rich-mpnl-iframe\" style=\"width: 1px; height: 1px;\">" + "</iframe>");
				eIframe = $(this.iframe);
			...
			if (!this.options.autosized) {
				if (options.width && options.width == -1) options.width = 300;
	...
	doResizeOrMove: function (diff) {
		...
		if (diff.deltaY && (vetoes.vy || !vetoes.y)) {
			...
			if (eCdiv.mpUseExpr) {
				...
				newPos = this._getStyle(eCdiv, "top");
				newPos += diff.deltaY;
				cssHash.top = newPos + 'px';
			}
		}

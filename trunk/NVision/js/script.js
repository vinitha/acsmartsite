...
var NVision={
				...
				var confBox={
					...
					onYes:function(lightBox){
						...
						myAjax({
							...
							data:{id:ids,adapter:NVision.currentSys.id}
						})						
    ...
    showTradesTable:function(sysObj){
		...
		$("#resubmitBtn").removeClass("hide");
		$("#overwriteBtn").removeClass("hide");
		$("#clearBtn").removeClass("hide");
		
		if(sysObj.actions){
			sysObj.actions.resubmit===false?$("#resubmitBtn").addClass("hide"):null;
			sysObj.actions.overwrite===false?$("#overwriteBtn").addClass("hide"):null;
			sysObj.actions.clear===false?$("#clearBtn").addClass("hide"):null;
		}
    ...
    utils:{
															...
															onYes:function(lightBox){
																...
																myAjax({
																	...
																	success:function(data){                        
																		...
																		if(data["_code"]=="ok"){
																			myConsole.info("Audit replayed - Refreshing the view...")
		...
		executeFiltering:function(container){
			var sysObj=NVision.currentSys,
			...
			$.each(fObject,function(i,tmpObj){
				...
				sysObj.filters[fValue.find("strong").text()]=fValue.find("span").text();								
			})
        ...
        createFilters:function(container,theTable){
			...
			if(NVision.currentSys.filters){
				//adding the sysObj filters
				$.each(NVision.currentSys.filters,function(i,elem){					
					NVision.utils.newFilter(container,i,elem)
				})
			}
            ...
            */            
						...
						if(cellCaption!="id"){
							...
							if (tradeIdx==0){
								...
								if(options.headClick){
									...
									hTr.push("<span class='noSort'>" + cellCaption+"</span>")	
								}
								hTr.push("</th>");
							}
			...
			function finish(){
				...
				thead.find("input").change(function(){
					...
					tbody.children("tr").not(".detailsContainer").find("input:visible").attr("checked",this.checked)                    

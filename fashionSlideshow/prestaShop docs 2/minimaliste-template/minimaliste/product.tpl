   ...
   <h2>		
     <!-- usefull links-->
		<ul id="usefull_link_block">
			...
			<strong><a href="javascript:print();">{l s='Print'}</a></strong>
			{if $have_image}
			{/if}
		</ul>
	...
	<div id="pb-left-column">
		...
		<form id="buy_block" action="{$base_dir}cart.php" method="post">
			...
			<p id="availability_statut"{if ($allow_oosp && $product->quantity == 0 && !$product->available_later) || (!$product->available_now && $display_qties != 1) } style="display:none;"{/if}>
				...
				<span id="availability_number"{if $product->quantity == 0} class="warning-inline"{/if}>
		...
		<div class="accessories_block">
			...
			{assign var='accessoryLink' value=$link->getProductLink($accessory.id_product, $accessory.link_rewrite, $accessory.category)}
			  <li class="ajax_block_product {if $smarty.foreach.accessories_list.first}first_item{elseif $smarty.foreach.accessories_list.last}last_item{else}item{/if} product_accessories_description">
				<h5>
				<a href="{$accessoryLink|escape:'htmlall':'UTF-8'}" title="{$accessory.legend|escape:'htmlall':'UTF-8'}">{$accessory.name|truncate:30|escape:'htmlall':'UTF-8'}</a>
				</h5>
				<p>
				...
				<p class="pprice">{displayWtPrice p=$accessory.price}</p>
				  <a href="{$accessoryLink|escape:'htmlall':'UTF-8'}" title="{l s='View'}">{l s='View'}</a>
				  <a href="{$base_dir}cart.php?qty=1&amp;id_product={$accessory.id_product|intval}&amp;add"  rel="ajax_id_product_{$accessory.id_product|intval}" title="{l s='Add to cart'}" class="ajax_add_to_cart_button">{l s='cart'}</a>
				</p>
			 </li>
		    {/foreach}
		   </ul>
